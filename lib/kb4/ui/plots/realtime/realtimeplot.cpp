    ...
    :AbstractPlot(parent)
{
    ...
    connect(qcp()->axisRect(), &QCPAxisRect::_wheelEvent, [this](QWheelEvent *event) {
        qDebug() << "im here" << event;
...
void RealTimePlot::configureAxesZoomAndDrag(bool configureXAxises)
{
    ...
    this->configureXAxises = configureXAxises;
    if (configureXAxises)
        axis << qcp()->xAxis;
